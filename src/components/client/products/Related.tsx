...
export function Relatedcard(props: Props) {
    ...
    let cartProduct = {
        ...
        name: item.name,
        price: item?.offerPrice || item?.onlinePrice,
    ...
    return (
        <div className='flex h-28 gap-2 justify-start items-center p-2 rounded-md bg-gray-100 hover:bg-blue-100 shadow'>
            ...
            <div className='grid h-full w-full content-between'>
                <Link href={`/product/${item?._id}`} className='text-sm font-normal truncate hover:text-sky-700'>
                    {item?.name}
                </Link>
                <div className='flex items-center justify-between gap-2'>
                    ...
                    <div className="flex items-center">
                        ...
                        <span className='px-2 lg:flex hidden'> {item.reviewCount} Reviews</span>
                    </div>
                    <button
                        className='p-1 flex items-center justify-center bg-sky-600 bg-opacity-10 rounded-md text-sky-600 hover:text-black'
                        type='button'
